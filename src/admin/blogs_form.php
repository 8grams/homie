<div
  x-init="
    //const d = new Drawer($el, { placement: 'right' })
    //setTimeout(() => d.show(), 500)
  "
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $hero_image = null;
    if (!$id) {
      $sql = 'INSERT INTO blogs (title, hero_image, content, slug, excerpt, header_code_injection, footer_code_injection, meta_title, meta_description, meta_keywords, meta_image, meta_url, category_id, created_at, updated_at) VALUES (:title, :hero_image, :content, :slug, :excerpt, :header_code_injection, :footer_code_injection, :meta_title, :meta_description, :meta_keywords, :meta_image, :meta_url, :category_id, :created_at, :updated_at)';
      $args = array_merge(
      ...
      $sql = 'UPDATE blogs SET title = :title, hero_image = :hero_image, content = :content, slug = :slug, excerpt = :excerpt, header_code_injection = :header_code_injection, footer_code_injection = :footer_code_injection, meta_title = :meta_title, meta_description = :meta_description, meta_keywords = :meta_keywords, meta_image = :meta_image, meta_url = :meta_url, category_id = :category_id, updated_at = :updated_at WHERE id = :id';
      $args = array_merge(
        array_diff_key($_POST, ['category' => null, 'tags' => null]),
        [
          'category_id' => $category,
  ...
  <form method="post" enctype="multipart/form-data">
    <div class="space-y-4">
      ...
      <div>
        ...
        <div
          class="w-full h-48 overflow-hidden cursor-pointer border border-gray-300 text-gray-300 rounded-lg dark:border-gray-600 dark:text-gray-600"
          x-data="{ file: null }"
          @click="$refs.input.click()">
          <input x-ref="input" @input="file = $el.files[0]" class="hidden" type="file" name="hero_image">
          <template x-if="!file">
            <div class="p-4 h-48 flex justify-center items-center">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="48"
                height="48"
          ...
          <template x-if="file">
            <img class="w-full h-48 object-cover" :src="URL.createObjectURL(file)" />
          </template>
        </div>
      </div>
      <div>
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Enter excerpt here"></textarea>
      </div>
      <div>
        <label
        ...
        <textarea
          ...
          rows="4"
          class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
          placeholder="Enter header code injection here"></textarea>
      </div>
      <div>
        <label
        ...
        <textarea
          ...
          rows="4"
          class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
          placeholder="Enter footer code injection here"></textarea>
      </div>
      ...
      <div x-data>
        ...
        <select
          data-categories="<?= htmlspecialchars($categories) ?>"
          x-init="
            const choices = new Choices($el, {
              choices: JSON.parse($el.dataset.categories).map(x => ({ value: x, text: x })),
              delimiter: ';',
              addChoices: true,
              maxItemCount: 1,
              addItems: true,
            ...
            $el.addEventListener('change', e => {
              $refs.category.value = e.detail.value
            })
          "
          type="text"
      ...
      <div x-data>
        ...
        <input type="hidden" x-ref="tags" name="tags">
        <select
          data-tags="<?= htmlspecialchars($tags) ?>"
          x-init="
            const choices = new Choices($el, {
              choices: JSON.parse($el.dataset.tags).map(x => ({ value: x, text: x })),
              delimiter: ';',
              addChoices: true,
              addItems: true,
              editItems: true,
            ...
            $el.addEventListener('change', e => {
              $refs.tags.value = choices.getValue().map(x => x.value).join(';')
            })
          "
          type="text"
